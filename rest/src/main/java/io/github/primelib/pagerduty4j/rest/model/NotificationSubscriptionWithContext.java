...
public class NotificationSubscriptionWithContext {
    ...
    public enum ResultEnum {
        ...
        private static final ResultEnum[] VALUES = values(); // prevent allocating a new array for every call to values()
        private final String value;

        @JsonCreator
        public static ResultEnum of(String input) {
            if (input != null) {
                for (ResultEnum v : VALUES) {
                    if (input.equalsIgnoreCase(v.value)) 
                        return v;
                }
            }
